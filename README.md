...
* Unsloth에서 지원되어 4bit QLoRA 기반 파인튜닝을 비교적 빠르게 진행할 수 있다.
...
전체 모델을 다시 학습하는 것은 많은 GPU 메모리와 시간이 필요하다. 따라서 본 프로젝트에서는 4bit QLoRA 방식을 사용하였다.

QLoRA는 4bit 양자화를 적용한 모델에 LoRA adapter를 추가하여 학습하는 방식이다. 이 방식은 전체 모델 파라미터를 모두 업데이트하지 않고 일부 adapter 파라미터만 학습하기 때문에, Google Colab T4 GPU와 같은 제한된 환경에서도 효율적으로 파인튜닝을 수행할 수 있다. 실제 코드에서도 `load_in_4bit=True`로 모델을 불러온 뒤 LoRA adapter를 추가하여 SFT 학습을 진행하였다.
...
수업 저장소에서 제공하는 `science_en` 데이터셋을 사용하였고, `unsloth/Qwen2.5-0.5B-Instruct` 모델에 4bit QLoRA 방식으로 학습을 진행하였다. 학습 과정에서는 training loss를 확인하여 모델이 실제로 학습되고 있음을 확인하였고, 학습 후에는 Base Model과 Fine-tuned Model의 응답을 비교하였다.
