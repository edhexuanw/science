...
| 모델 저장     | LoRA adapter를 `lora_model/` 폴더로 저장하였다 |
| 모델 export | GGUF 형식으로 export하여 `.gguf` 파일을 생성하였다  |

---

## 9. 실행 화면 및 스크린샷

학습 과정과 추론 결과를 확인하기 위해 아래와 같은 스크린샷을 저장하였다.

| 항목              | 파일 경로                           |
| --------------- | ------------------------------- |
| 학습 로그           | `images/training_log.png`       |
| 학습 완료 결과        | `images/training_result.png`    |
| 모델 비교           | `images/model_comparison.png`   |
| 추론 예시           | `images/inference_example.png`  |
| LoRA adapter 저장 | `images/export_result.png`      |
| GGUF export 결과  | `images/gguf_export_result.png` |
...
파인튜닝 후 모델은 베이스 모델보다 더 짧고 명확한 답변을 생성하는 것을 목표로 하였으며, 특히 과학 개념을 초보 학습자가 이해하기 쉬운 표현과 간단한 예시로 바꾸는 데 초점을 두었다.

또한 학습 후 LoRA adapter를 저장하였고, 추가로 GGUF 형식으로 export하여 로컬 실행에 활용할 수 있는 `.gguf` 파일을 생성하였다. 이를 통해 베이스 모델 선택, 데이터셋 설계, LoRA/QLoRA 학습, 베이스 대비 개선 확인, export까지 이어지는 파인튜닝 파이프라인을 수행하였다.
